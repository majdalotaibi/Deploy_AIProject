...
page = st.sidebar.selectbox("اختر صفحة:", ["الرئيسية", "تعرف على النباتات", "ماهي نبتتي؟", "ازرع نبتتك", "افحص نبتتك", "الأعضاء"])

# محتوى الصفحات
if page == "الرئيسية":
    # إضافة محتوى الصفحة الرئيسية
    st.markdown("<h1>مرحبا بك في غرسة</h1>", unsafe_allow_html=True)
    # يمكنك إضافة المزيد من المحتوى هنا

elif page == "تعرف على النباتات":
    exec(open(os.path.join(script_dir, "pages", "plants_info.py")).read())

elif page == "ماهي نبتتي؟":
    exec(open(os.path.join(script_dir, "pages", "what_is_my_plant.py")).read())

elif page == "ازرع نبتتك":
    exec(open(os.path.join(script_dir, "pages", "plant_your_plant.py")).read())

elif page == "افحص نبتتك":
    exec(open(os.path.join(script_dir, "pages", "check_your_plant.py")).read())

elif page == "الأعضاء":
    exec(open(os.path.join(script_dir, "pages", "team_members.py")).read())
